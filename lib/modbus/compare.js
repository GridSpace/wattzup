...
const registers = require('./register-map.js');
...
function findClosestRow(packetTimestamp, csvData) {
    ...
    for (const row of csvData) {
        // Parse the CSV time string directly, creating a local Date object.
...
console.log(`Regex matched: year=${year}, month=${month}, day=${day}, hour=${hour}, minute=${minute}, second=${second}`);

// Create a UTC date object to ensure correct comparison with CSV data.
const packetTime = `${year}-${month}-${day} ${hour}:${minute}`;
const packetTimestamp = new Date(packetTime);
console.log(`Packet Timestamp: ${packetTimestamp} from ${packetTime}`);
