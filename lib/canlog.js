...
async function load() {
    ...
    function addscan(act, pos, len) {
        ...
        const pval = (arec[pkey] = (arec[pkey] || 0) + 1);
    }
    ...
    function matchscan(str, dat, scanv, addscan) {
        ...
        for (let i=0; i<dat.length-3; i++) {
            const val = args.signed ? bufr.readInt32LE(i) : bufr.readUint32LE(i);
            if (val >= lo && val <= hi) {
                addscan(str, i, 4);
                match = true;
            }
    ...
    for await (let line of lines) {
        ...
        // old school stream base types 004 and 005
        if (cmd[0] === '0') {
            if (args.regen) {
            ...
            const out = [
                ...
                color(rrf, FG.green),
                color(seq, FG.GREEN),
                color(group(unk,2), FG.green),  // rest of header
                color(hex, FG.BLUE),            // message payload
        ...
        /**
         * messages 100, 101, 102 come as a group (start, body, end)
         * messages 004, 008, 00C come as a group (start, body, end)
         * 106 amd 10A are stand-alone
         */
        switch (cmd) {
            case '100':
            case '120':
            ...
            case '102':
                if (msgs[stream]) {
                    ...
                    if (args.hhas && hdr.indexOf(args.hhas) < 0) {
                        continue;
                    }
                    ...
                    if (args.scan && !matchscan(str, dat, scanv, addscan)) {
                        continue;
                    }
                    if (args.meta) {
