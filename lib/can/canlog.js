...
async function load() {
    ...
    const rtys = args.rtys?.split(',') || args.rty?.split(',');
    const typs = args.typs?.split(',') || args.typ?.split(',');
    const ty2s = args.ty2s?.split(',') || args.ty2?.split(',');
    const rrfs = args.rrfs?.split(',') || args.rrf?.split(',');
    const dv4s = args.dv4s?.split(',') || args.dv4?.split(',');
    const nods = args.nodes?.split(',') || args.node?.split(',');
    ...
    function write_group(group) {
        ...
        if (fnout !== regenlf) {
            ...
            if (fs.existsSync('sample.on') && !sample) {
                sample = true;
                log('enabling sampling');
            } else if (fs.existsSync('sample.off') && sample) {
                sample = false;
